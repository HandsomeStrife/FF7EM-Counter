...
def calculate_intervals(blue_percentages, red_percentages, fps):
    intervals = []
    start_time = None

    for i, (blue, red) in enumerate(zip(blue_percentages, red_percentages)):
        if blue > red:
...
# Assume a frame rate (fps) for the video
fps = 30  # Modify this value based on your video's frame rate
intervals = calculate_intervals(blue_percentages, red_percentages, fps)

intervals.reverse()
intervals = [interval for interval in intervals if interval > 0.9]

# round all the intervals to 2 decimal places
